#' Main Graphs for Multiblock Weighted Covariate analysis (MB-WCov)
#'
#' Plot the main graphs for MB-WCov.
#'
#' @param x  An object of class \code{MBWCov}.
#' @param axes  A vector of length 2 which specifies the global components to plot (by default the first two).
#' @param graphtype  Type of graph to plot. Either "saliences", "globalscores", "blockscores","globalcor","blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit" or "contrib".
#' Refer to the details section.
#' @param select  Selection of elements to plot (by default, select=NULL). Refer to the details section.
...
#'
#'
#' @details The arguments graphtype and select are used as follow.\cr
...
#'
#'
#' @return Returns graphs showing relationships between the explanatory blocks of variables, the projection of individuals in both global and block components, the correlations of variables with the global and block components,
...
#'
#' @export
#' @importFrom  ggplot2 ggplot aes theme_bw labs theme element_text element_blank geom_point geom_hline geom_vline geom_segment arrow unit geom_path scale_color_manual scale_fill_manual xlab ylab xlim ylim
#' @importFrom  ggrepel geom_text_repel
#' @importFrom  grDevices dev.new

# Plot method for MBWCov class

plot.MBWCov <- function(x, axes = c(1, 2), graphtype = c("saliences", "globalscores", "blockscores", "globalcor", "blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit", "contrib"), select=NULL, max.overlaps = 20 , xlim = NULL, ylim = NULL, title=NULL, color=NULL, ...){
  ...
  if (!graphtype %in% c("saliences", "globalscores", "blockscores","globalcor", "blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit", "contrib")) stop("\n\ngraphtype must be either saliences, globalscores, blockscores, globalcor, blockcor, explY, cumexplY, explX, cumexplX, crit or contrib. \n \n")
  ...
  if (graphtype=="saliences"){
    ...
    if (is.null(title))
      title=paste("MB-WCov: Specific weights of X blocks on dimensions", axes[1]," and ", axes[2])
    else title<- title
    #pdf(file="Saliences.pdf")
    dev.new()
  ...
  if (graphtype=="globalscores"){
    ...
    if (is.null(title))
      title=paste("MB-WCov: Projection of individuals in dimensions", axes[1],"and", axes[2])
    else title<- title
  ...
  if (graphtype=="blockscores"){
    ...
    for (i in select) {
      ...
      p <- ggplot(as.data.frame(Res$Block$T.b[,,i]), aes(c1, c2))+
        theme_bw() + labs(title = paste("MB-WCov: Projection of individuals of block",i,"in dimensions", axes[1],"and", axes[2]))+  xlab(xlab) +  ylab(ylab)+
  ...
  if (graphtype=="globalcor"){
    ...
    if (is.null(title))
      title=paste("MB-WCov: Correlation of variables with dimensions", axes[1],"and", axes[2])
    else title<- title
  ...
  if (graphtype=="blockcor"){
    ...
    for (i in select) {
      ...
      p <- ggplot(data=as.data.frame(Res$Block$blockcor[[i]]), aes(x=c1, y=c2))+
        theme_bw() + labs(title = paste("MB-WCov: Correlation of variables of block",i," with dimensions", axes[1],"and", axes[2]))+
  ...
  if (graphtype=="explY"){
    ...
    if (is.null(ylim))
      ylim<- c(0,max(Res$cumexplained[1:select,3]))
    else ylim=ylim
    if (is.null(title))
      title=paste("MB-PCA: Scree plot of Y block")
    else title<- title

    dev.new()
    p <- ggplot(as.data.frame(Res$cumexplained[1:select,]), aes(x=row.names(Res$cumexplained[1:select,]), y=Res$cumexplained[1:select,3]))+
      geom_bar(stat="identity", width=0.2,fill="blue")+
      theme_bw() + labs(title = title, x="Dimension",y="Percentages of inertia explained in Y block")+ ylim(ylim)+
      theme(plot.title = element_text(hjust = 0.5)) # center the title
    print(p)
    #dev.off()
  }

  if (graphtype=="cumexplY"){
    if (is.null(select)) {
      select=Res$components[2]
    } else {
      select=select
    ...
    if (is.null(ylim))
      ylim<- c(0,max(Res$cumexplained[1:select,4]))
    else ylim=ylim
    if (is.null(title))
      title=paste("MB-WCov: Cumulative percentages of inertia explained in Y block")
    else title<- title
    dev.new()

    p <- ggplot(as.data.frame(Res$cumexplained[1:select,]), aes(x=row.names(Res$cumexplained[1:select,]), y=Res$cumexplained[1:select,4]))+
      geom_bar(stat="identity", width=0.2,fill="blue")+
      theme_bw() + labs(title = title, x="Dimension",y="Cumulative percentages of inertia explained in Y block")+ ylim(ylim)+
      theme(plot.title = element_text(hjust = 0.5)) # center the title
    print(p)
    #dev.off()
  }
  ...
  if (graphtype=="explX"){
    ...
    if (is.null(title))
      title=paste("MB-PCA: Scree plot of X blocks")
    else title<- title
    ...
    p <- ggplot(as.data.frame(Res$cumexplained[1:select,]), aes(x=row.names(Res$cumexplained[1:select,]), y=Res$cumexplained[1:select,1]))+
      geom_bar(stat="identity", width=0.2,fill="blue")+
      theme_bw() + labs(title = title, x="Dimension",y="Percentages of inertia explained in all the X blocks")+ ylim(ylim)+
      theme(plot.title = element_text(hjust = 0.5)) # center the title
    print(p)
    #dev.off()
  }

  if (graphtype=="cumexplX"){
    if (is.null(select)) {
      select=Res$components[2]
    } else {
      select=select
    ...
    if (is.null(title))
      title=paste("MB-WCov: Cumulative percentages of inertia explained in X blocks")
    else title<- title
    dev.new()

    p <- ggplot(as.data.frame(Res$cumexplained[1:select,]), aes(x=row.names(Res$cumexplained[1:select,]), y=Res$cumexplained[1:select,2]))+
      geom_bar(stat="identity", width=0.2,fill="blue")+
      theme_bw() + labs(title = title, x="Dimension",y="Cumulative percentages of inertia explained in X blocks")+ ylim(ylim)+
      theme(plot.title = element_text(hjust = 0.5)) # center the title
    print(p)
    #dev.off()
  }
  ...
  if (graphtype=="crit"){
    ...
    if (is.null(title))
      title=paste("MB-WCov: Values of the maximization criterion")
    else title<- title
  ...
  if (graphtype=="contrib"){
    ...
    if (is.null(title))
      title=paste("MB-WCov: Contribution of X blocks to the determination of dimensions")
    else title<- title
    ...
    p <- ggplot(data, aes(x=Dimension, y=Value, fill=Block)) +
      ...
      theme_bw() + labs(title = title, x="Dimension",y="Contribution of X blocks to the determination of various dimensions")+
      theme(plot.title = element_text(hjust = 0.5)) # center the title
    print(p)
    #dev.off()
  }
