#' Main Results for Multiblock Partial Least Squares (MB-PLS) regression
#'
#' Print the main results for MB-PLS regression.
#'
...
print.MBPLS<-function (x, ...)
{
  ...
  cat("-------------------------------------------------------------------------\n")
  cat("* Results for Multiblock Partial Least Squares regression                               \n")
  cat("-------------------------------------------------------------------------\n")
  ...
  #cat("------------------------------------------------------------------------\n")
  cat("-------------------------------------------------------------------------\n")
  Res <- array("", c(14, 5), list(1:14, c("|","NAME OF OBJECT", "|", "DESCRIPTION OF OBJECT", "|")))
  cat("-------------------------------------------------------------------------\n")
  ...
  Res[6, ] <- c("|", "$contrib","|", "Contribution of blocks to the global components", "|")
  Res[7, ] <- c("|", "$T","|", "Global components", "|")
  Res[8, ] <- c("|", "$C","|", "Compromise matrix", "|")
  Res[9, ] <- c("|", "$U","|", "Components associated with Y", "|")
  Res[10, ] <- c("|", "$globalcor","|", "Correlations variables-global components", "|")
  Res[11, ] <- c("|", "$cor.g.b","|", "Correlations global components-block components", "|")
  Res[12, ] <- c("|", "$betaY","|", "Regression coefficients", "|")
  Res[13, ] <- c("|", "$Block$T.b","|","Block components", "|")
  Res[14, ] <- c("|", "$Block$blockcor","|", "Correlations variables-block components", "|")
  print(Res[1:14, ],quote=FALSE)
  cat("-------------------------------------------------------------------------\n\n")
}
