...
#'
#' MB-WCov applied to multiblock quantitative variables.
#'
...
#' If scaleY="sd", each variable of the response block is divided by its standard deviation.
...
#' @param plotgraph  Boolean (TRUE/FALSE). If TRUE (default), graphs depicting saliences, scores of individuals, correlations of variables with the global components and contributions of blocks of variables to the determination of global components are displayed.
...
#'
#' @seealso \code{\link{print.MBWCov}}    \code{\link{plot.MBWCov}}    \code{\link{summary.MBWCov}}
#'
#' @examples
#' data(ham)
#' X=ham$X
...
MBWCov <- function(X, Y, group, algo="eigen", ncompprint=NULL, scale="none", scaleY="none", option="uniform", optionY="uniform", nstart=10, threshold=1e-8, plotgraph=TRUE, axes=c(1,2)){
  ...
  saliences    <- LAMBDA <- NNLAMBDA <- matrix(0,ntab,ncomp)   # Specific weights for each block and each dimension
  ...
  for (comp in 1:ncomp)  {                           # Iterative computation of the various components
    ...
    if  (algo=="eigen"){
      ...
      while(deltacrit>threshold) {
        ...
        Q       <- matrix(0,nrow=ncolY,ncol=ncolY)

        for(j in 1:ntab) {
          Q      <- Q + lambda[j]*t(Y)%*%tcrossprod(X[[j]])%*%Y
        }
        reseig  <- eigen(Q)
        nu       <- reseig$vectors[,1]
        optimalcrit[comp]    <- reseig$values[1]#/nind
        LAMBDA[,comp]  <- matrix(sapply(seq_len(ntab),function(j){t(nu)%*%t(Y)%*%tcrossprod(X[[j]])%*%Y%*%nu}),nrow=ntab,byrow=FALSE)
    ...
    for (i in seq_len(nstart)) {
      ...
      while(deltacrit>threshold)  {
        ...
        LAMBDA[,comp]  <- matrix(sapply(seq_len(ntab),function(j){t(u)%*%tb[,j]}),nrow=ntab,byrow=FALSE)
        NNLAMBDA[,comp]<- LAMBDA[,comp]               # Non normalized specific weights
        LAMBDA[,comp]  <- normv(LAMBDA[,comp])
        t           <- rowSums(t(t(tb)*LAMBDA[,comp]))
        t           <- normv(t)
        nu          <- normv(crossprod(Y,t))
        criterion   <- sum(sapply(seq_len(ntab),function(j){LAMBDA[j,comp]*t(u)%*%tb[,j]}))
        deltacrit   <- criterion- critt
        critt       <- criterion
        iNIPALS     <- iNIPALS + 1
      } # end while
    ...
    for(j in 1:ntab) {
      ...
      T[,comp]        <- T[,comp] + LAMBDA[j,comp]*T.b[,comp,j]
    }
    T[,comp]        <- normv(T[,comp])
    ...
    Px[,comp]  <- unlist(sapply(1:ntab, function(j){t(X[[j]])%*%T[,comp]}))
  ...
  if (plotgraph) {
    plot.MBWCov(res,graphtype="saliences", axes=axes)
    plot.MBWCov(res,graphtype="globalscores", axes=axes)
    plot.MBWCov(res,graphtype="globalcor", axes=axes)
    plot.MBWCov(res,graphtype="contrib", axes=axes)
   }
  return(res)
}
