#' Main Graphs for Multiblock Partial Least Squares (MB-PLS) regression
#'
#' Plot the main graphs for MB-PLS regression.
#'
#' @param x  An object of class \code{MBPLS}.
#' @param axes  A vector of length two which specifies the global components to plot (by default the first two).
#' @param graphtype  Type of graph to plot. Either "globalscores", "blockscores","globalcor","blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit" or "contrib".
...
#'
#'
#' @return Returns graphs showing the projection of individuals in both global and block components, the correlations of variables with the global and block components,
...
# Plot method for MBPLS class

plot.MBPLS <- function(x, axes = c(1, 2), graphtype = c("globalscores", "blockscores", "globalcor", "blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit","contrib"), select=NULL, max.overlaps = 20 , xlim = NULL, ylim = NULL, title=NULL, color=NULL, ...){
  ...
  if (!graphtype %in% c("globalscores", "blockscores","globalcor", "blockcor", "explY", "cumexplY", "explX", "cumexplX", "crit", "contrib")) stop("\n\ngraphtype must be either globalscores, blockscores, globalcor, blockcor, explY, cumexplY, explX, cumexplX, crit or contrib. \n \n")
  ...
  if (graphtype=="globalscores"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Projection of individuals in dimensions", axes[1],"and", axes[2])
    else title<- title
  ...
  if (graphtype=="blockscores"){
    ...
    for (i in select) {
      ...
      p <- ggplot(as.data.frame(Res$Block$T.b[,,i]), aes(c1, c2))+
        theme_bw() + labs(title = paste("MB-PLS: Projection of individuals of block",i,"in dimensions", axes[1],"and", axes[2]))+  xlab(xlab) +  ylab(ylab)+
  ...
  if (graphtype=="globalcor"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Correlation of variables with dimensions", axes[1],"and", axes[2])
    else title<- title
  ...
  if (graphtype=="blockcor"){
    ...
    for (i in select) {
      ...
      p <- ggplot(data=as.data.frame(Res$Block$blockcor[[i]]), aes(x=c1, y=c2))+
        theme_bw() + labs(title = paste("MB-PLS: Correlation of variables of block",i," with dimensions", axes[1],"and", axes[2]))+
  ...
  if (graphtype=="cumexplY"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Cumulative percentages of inertia explained in Y block")
    else title<- title
    dev.new()
  ...
  if (graphtype=="cumexplX"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Cumulative percentages of inertia explained in X blocks")
    else title<- title
    dev.new()
  ...
  if (graphtype=="crit"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Values of the maximization criterion")
    else title<- title
   ...
   if (graphtype=="contrib"){
    ...
    if (is.null(title))
      title=paste("MB-PLS: Contribution of X blocks to the determination of dimensions")
    else title<- title
